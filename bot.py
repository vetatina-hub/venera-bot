...
async def show_result(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🌀 И самое ценное — Пространство трансформации. Здесь проходят живые разборы, "
        f"и ты можешь быть их частью: как фигура ты получаешь собственные трансформации "
        f"в каждом разборе. Присоединяйся 👇"
    )
