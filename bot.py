...
async def show_result(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    invite_text = (
        f"🌀 Пока ждёшь практику — загляни в Пространство трансформации\n\n"
        f"Здесь проходят живые разборы в квантовом поле. Даже если ты просто "
        f"наблюдаешь как фигура — ты сама получаешь свою трансформацию в моменте.\n\n"
        f"Заходи, знакомься, будь частью 👇"
    )
